  ...
  class SuperMembership
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `XeroRuby::PayrollAu::SuperMembership` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `XeroRuby::PayrollAu::SuperMembership`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @super_fund_id.nil?
        invalid_properties.push('invalid value for "super_fund_id", super_fund_id cannot be nil.')
      end
    ...
    def valid?
      return false if @super_fund_id.nil?
    ...
    def hash
      [super_membership_id, super_fund_id, employee_number].hash
    end
